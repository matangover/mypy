...
def write_header(file , module_name  = None,
                 pyversion   = (3, 5))  :
    if module_name:
        if pyversion[0] >= 3:
...
def infer_sig_from_docstring(docstr , name )   :
    ...
    if m:
        ...
        return get_boost_sig(m.group(1)), m.group(2).rstrip()

    # try to not match return type
    m = re.match(sig_match, docstr)
    if m:
        return get_boost_sig(m.group(1)), 'Any'
    return None

def get_boost_sig(sig):
    """
    >>> print ProductionData.SetParameter.__doc__
    SetParameter( (ProductionData)arg1, (ProcessorParameter)arg2, (object)arg3) -> None
    """
    # Strip away parentheses
    args_str = sig[1:-1]
    args = args_str.split(",")
    args = args[1:]
    pyargs = [re.sub(r'\((.+)\)(.+)', r'\2: \1', arg.strip()) for arg in args]
    return '(%s)' % ', '.join(pyargs)

def infer_prop_type_from_docstring(docstr )  :
    if not docstr:
        return None
